import React, { useEffect, useMemo, useRef, useState } from "react";
import css from "./Talk_Groq.module.css";
import AI_Calling_img from "../../Assets/Image/ai_calling.jpg";
import MIC_IMG from "../../Assets/Image/mic_6.png";
import { useNavigate, Link } from "react-router-dom";
import RIGHT_ARROW_IMG from "../../Assets/Image/right_arrow.png";
import MIC_VID from "../../Assets/Video/mic.mp4";
// import MIC_VID from "../../Assets/Video/mic.png";
import { v4 as uuidv4 } from "uuid";
import Markdown from "react-markdown";

function Home() {
  const nav = useNavigate();
  const [transcript, settranscript] = useState(null);
  const [ismic, setismic] = useState(false);
  const [audioStream, setAudioStream] = useState(null);
  const [socket, setSocket] = useState(null);
  const [lang, setlang] = useState("en-US");
  const [audioBlobs, setaudioBlobs] = useState([]);
  const [cansend, setcansend] = useState(true);
  const [audioUrl, setAudioUrl] = useState(null);
  const [currentAudioIndex, setCurrentAudioIndex] = useState(0);
  const [stt, setstt] = useState("deepgram");
  const [tts, settts] = useState("deepgram");
  const [isfinal, setisffinal] = useState("llama3-70b-8192");
  const [chat_model, setchat_model] = useState("llama3-8b-8192");
  const [endp, setend] = useState(20);
  const [can_endp_change, setcan_end_change] = useState(true);
  const [isprompt, setisprompt] = useState(false);
  const [chat, setchat] = useState([]);
  const [mic_vid, setmic_vid] = useState(true);
  const [chat_prompt, setchat_prompt] = useState(sessionStorage?.getItem("prompt")?sessionStorage?.getItem("prompt"):"");
  let isplaying = false;
  const audioRef = useRef(null);
  const [isPlaying, setIsPlaying] = useState(false);
  const videoRef = useRef(null);
  const msgref = useRef(null);
  const msgref2 = useRef(null);

  // useEffect(() => {
  //   console.log("Scroll into view.");
  //   setTimeout(() => {
  //     msgref.current?.scrollIntoView();
  //     msgref2.current?.scrollIntoView();
  //   }, 2000);
  //   msgref.current?.scrollIntoView();
  //   msgref2.current?.scrollIntoView();
  // }, [chat]);

  const sessionId = uuidv4();

  const scrollToTop = () => {
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  useEffect(() => {
    if (!ismic && window.location.href.includes("localhost")) {
      scrollToTop();
    }
    if(sessionStorage.getItem("prompt")){
      setchat_prompt(sessionStorage?.getItem("prompt"));
    }
  }, []);

  const playAudio = async (index) => {
    try {
      if (index >= 0 && index < audioBlobs.length) {
        isplaying = true;
        setCurrentAudioIndex(index);
        // audioRef.current.src = audioBlobs.shift().url;
        audioRef.current.src = audioBlobs[index].url;
        const audioelement = new Audio(audioBlobs[index].url);
        //   await audioelement.play();
        await audioRef.current.play();
        console.log("Playing  " + index);

        audioRef.current.onended = () => {
          if (index <= audioBlobs.length) {
            // audioBlobs.shift();
            playAudio(index + 1);
          } else {
            setcansend(true);
            isplaying = false;
            console.log("Stoped at", index);
            console.log("Cansend  ", cansend);
            setaudioBlobs([]);
            console.log("Audio blobs length :", audioBlobs.length);
          }
          // console.log("next");
          // console.log(index);
        };
        //   audioelement.onended = () => {
        //     if (index <= audioBlobs.length) {
        //       // audioBlobs.shift();
        //       playAudio(index + 1);
        //     } else {
        //       setcansend(true);
        //       isplaying = false;
        //       console.log("Stoped at", index);
        //       console.log("Cansend  ", cansend);
        //     }
        //     // console.log("next");
        //     // console.log(index);
        //   };
      } else {
        console.log("index  " + index);
        //   if (index === 0) {
        //     console.log("condition");
        //     // const aud = new Audio(firsturl);
        //     audioRef.current.src = firsturl;
        //     // await aud.play();
        //     await audioRef.current.play();
        //   }
        console.log("Empty");
        setcansend(true);
        isplaying = false;
        setaudioBlobs([]);
        console.log("Audio blobs length :", audioBlobs.length);
      }
    } catch (err) {
      console.log("Error in playing sound : ", err);
    }
  };

  const handlemicchange = async () => {
    setIsPlaying((prevState) => !prevState);
    const video = videoRef.current;
    if (video.paused) {
      video.play();
      setIsPlaying(true);
    } else {
      video.pause();
      setIsPlaying(false);
      if (videoRef.current) {
        videoRef.current.currentTime = 0;
      }
    }

    if (stt === "whisperr") {
    } else {
      if (!ismic) {
        settranscript(null);
        await startAudioStream();
      } else {
        stopAudioStream();
      }
    }
    ismic ? setismic(false) : setismic(true);
    !ismic ? setcan_end_change(false) : setcan_end_change(true);
  };

  useEffect(() => {
    // Initialize WebSocket connection
    let ws;

    if (window.location.href.includes("localhost")) {
      ws = new WebSocket(`ws://${process.env.REACT_APP_WEBSOCKET_URL}/audio`);
    } else {
      ws = new WebSocket(`wss://${process.env.REACT_APP_WEBSOCKET_URL}/audio`);
    }
    ws.onopen = () => {
      setSocket(ws);
      console.log("WebSocket connected");
      ws.send("Language " + lang);
      console.log(sessionId);
      ws.send("id " + sessionId);
      console.log(stt);
      ws.send("stt" + stt);
      console.log(tts);
      ws.send("tts" + tts);
      console.log(isfinal);
      ws.send("isfinal_model" + isfinal);
      console.log(chat_model);
      ws.send("chat_model" + chat_model);
      ws.send("prompt" + chat_prompt);
      console.log(endp);
      ws.send("endpointing" + endp);
    };

    ws.onmessage = async (event) => {
      const data = JSON.parse(event.data);
      console.log("WebSocket Response:", typeof data);

      if (data.type == "transcript") {
        settranscript((prevTranscript) =>
          prevTranscript ? prevTranscript + data.msg : data.msg
        );
        if (data.msg) {
          let a = chat;
          a.push({ speaker: "USER", message: data.msg });
          setchat(a);
        }
      }

      if (data.type == "pop") {
        if (data.msg === "pop") {
          let a = chat;
          a.pop();
          setchat(a);
        }
      }

      if (data.type == "llm") {
        if (data.msg) {
          let a = chat;
          a.push({ speaker: "AI", message: data.msg });
          setchat(a);
        }
      }

      if (data.type == "file") {
        if (data.number >= 0) {
          try {
            const url = `${process.env.REACT_APP_BACKEND_URL}/audio_files/${sessionId}/audio${data.number}.mp3`;
            console.log("Url : ", url);
            console.log("File Number : " + data.number);
            console.log("Audio blobs length :", audioBlobs.length);

            if (url) {
              let a = audioBlobs;
              a.push({ id: 1, url: url });
              setaudioBlobs(a);
              console.log(audioBlobs);
              if (!isplaying) {
                console.log("!playing");
                isplaying = true;
                setcansend(false);
                console.time("Startplaying");
                playAudio(data.number);
              }
            }
          } catch (err) {
            console.log("Error in fetching : ", err);
            // throw new Error("Network response was not ok");
          }
        }
      }
    };

    ws.onclose = () => {
      console.log("Websocket Close");
    };

    ws.onerror = (err) => {
      console.log("Websocket Error", err);
    };

    // Cleanup function
    return () => {
      ws.close();
    };
  }, []);

  useEffect(() => {
    console.log(lang);
    if (socket && socket.readyState === WebSocket.OPEN) {
      socket.send("Language " + lang);
    }
  }, [lang]);
  useEffect(() => {
    console.log(lang);
    if (socket && socket.readyState === WebSocket.OPEN) {
      socket.send("stt" + stt);
    }
  }, [stt]);
  useEffect(() => {
    console.log(tts);
    if (socket && socket.readyState === WebSocket.OPEN) {
      socket.send("tts" + tts);
    }
  }, [tts]);
  useEffect(() => {
    console.log(isfinal);
    if (socket && socket.readyState === WebSocket.OPEN) {
      socket.send("isfinal_model" + isfinal);
    }
  }, [isfinal]);
  useEffect(() => {
    console.log(chat_model);
    if (socket && socket.readyState === WebSocket.OPEN) {
      socket.send("chat_model" + chat_model);
    }
  }, [chat_model]);
  useEffect(() => {
    console.log(chat_model);
    if (socket && socket.readyState === WebSocket.OPEN) {
      socket.send("prompt" + chat_prompt);
    }
  }, [chat_prompt]);

  useEffect(() => {
    console.log(endp);
    if (socket && socket.readyState === WebSocket.OPEN) {
      socket.send("endpointing" + endp);
    }
  }, [endp]);

  function floatTo16BitPCM(input) {
    const output = new Int16Array(input.length);
    for (let i = 0; i < input.length; i++) {
      const sample = Math.max(-1, Math.min(1, input[i]));
      output[i] = sample < 0 ? sample * 0x8000 : sample * 0x7fff;
    }
    return output.buffer;
  }

  const startAudioStream = async () => {
    try {
      console.log("Starting audio stream...");
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      setAudioStream(stream);

      if (!socket || socket.readyState !== WebSocket.OPEN) {
        console.error("WebSocket connection is not open.");
        return;
      }

      socket.send("Start");
      socket.send("Language " + lang);
      console.log(stt);
      socket.send("stt" + stt);
      console.log(tts);
      socket.send("tts" + tts);

      const audioContext = new AudioContext({
        sampleRate: 16000,
      });
      const audioInput = audioContext.createMediaStreamSource(stream);
      const bufferSize = 2048;
      const scriptProcessorNode = audioContext.createScriptProcessor(
        bufferSize,
        1,
        1
      );

      scriptProcessorNode.onaudioprocess = (e) => {
        const inputData = e.inputBuffer.getChannelData(0);
        const l16Data = floatTo16BitPCM(inputData);
        socket.send(l16Data);
      };

      audioInput.connect(scriptProcessorNode);
      scriptProcessorNode.connect(audioContext.destination);
    } catch (error) {
      console.error("Error accessing microphone:", error);
    }
  };

  const stopAudioStream = async () => {
    if (audioStream) {
      await audioStream.getTracks().forEach((track) => track.stop());
      setAudioStream(null);
      if (socket) {
        socket.send("Stop");
      }
    }
  };

  return (
    <>
      <div className={`${css.main} ${isprompt?css.brightness:null}`}>
        <audio
          ref={audioRef}
          controls
          autoPlay
          allow="autoplay"
          className={css.audio}
        >
          <source src="" type="audio/mpeg" />
          Your browser does not support the audio element.
        </audio>

        <div className={css.try_headings}>
          {/* <div className={css.micc}>
            <span className={css.span}>Give it a Try</span>
          </div> */}
        </div>

        <div className={css.options}>
          <div className={css.options}>
            <div class={css.select_wrapper}>
              <label>
                <b> S-T-T : </b>
              </label>
              <select
                class={css.language_select}
                onChange={(e) => {
                  setstt(e.target.value);
                }}
              >
                <option value="deepgram">Deepgram</option>
                <option value="whisper">Whisper</option>
              </select>
            </div>
            <div class={css.select_wrapper}>
              <label>
                <b> T-T-S : </b>
              </label>
              <select
                class={css.language_select}
                onChange={(e) => {
                  settts(e.target.value);
                }}
              >
                <option value="deepgram">Deepgram</option>
                <option value="playht">Play.ht</option>
              </select>
            </div>
            <div class={css.select_wrapper}>
              <label>
                <b> Language : </b>
              </label>
              <select
                class={css.language_select}
                onChange={(e) => {
                  setlang(e.target.value);
                }}
              >
                <option value="en-US">English</option>
                <option value="hi">Hindi</option>
                <option value="es">Spanish</option>
                <option value="fr">French</option>
                <option value="de">German</option>
              </select>
            </div>
          </div>

          <hr style={{ color: "black", width: "100%", marginTop: "25px" }} />

          <div className={css.options}>
            <div class={css.select_wrapper}>
              <label>
                <b>Is Final Model : </b>
              </label>
              <select
                class={css.language_select}
                onChange={(e) => {
                  setisffinal(e.target.value);
                }}
              >
                {/* <option value="meta-llama/Llama-3-70b-chat-hf">meta-llama/Llama-3-70b-chat-hf</option> */}
                {/* <option value="meta-llama/Llama-3-8b-chat-hf">meta-llama/Llama-3-8b-chat-hf</option> */}
                <option value="llama3-70b-8192">llama3-70b-8192</option>
                <option value="llama3-8b-8192">llama3-8b-8192</option>
                {/* <option value="gemma-7b-it">gemma-7b-it</option> */}
              </select>
            </div>
            <div class={css.select_wrapper}>
              <label>
                <b>Chat Model : </b>
              </label>
              <select
                class={css.language_select}
                onChange={(e) => {
                  setchat_model(e.target.value);
                }}
              >
                {/* <option value="meta-llama/Llama-3-70b-chat-hf">meta-llama/Llama-3-70b-chat-hf</option> */}
                {/* <option value="meta-llama/Llama-3-8b-chat-hf">meta-llama/Llama-3-8b-chat-hf</option> */}
                <option value="llama3-8b-8192">llama3-8b-8192</option>
                <option value="llama3-70b-8192">llama3-70b-8192</option>
                {/* <option value="gemma-7b-it">gemma-7b-it</option> */}
              </select>
            </div>

            <div class={css.select_wrapper}>
              <label>
                <b>Endpointing : </b>
              </label>
              <input
                disabled={can_endp_change ? false : true}
                type="Number"
                placeholder="20"
                min={10}
                class={css.language_select}
                onChange={(e) => {
                  setend(e.target.value);
                }}
              ></input>
            </div>
            <div class={css.select_wrapper}>
              <button
                className={`${css.prompt}`}
                onClick={() => {
                  setisprompt(true);
                }}
              >
                Add Call Instructions
              </button>
            </div>
          </div>
        </div>

        <div className={css.try}>
          {/* <div className={!ismic ? css.mic_boxx : css.mic_boxxx}>
          {
              <img
                src={MIC_VID}
                onClick={handlemicchange}
              />
            }
          </div> */}

          <div className={!ismic ? css.mic_boxx : css.mic_boxxx}>
            {
              <video
                ref={videoRef}
                src={MIC_VID}
                muted
                loop
                onClick={handlemicchange}
                autoPlay={isPlaying}
              />
            }
          </div>
          <div className={css.transcript_box}>
            <h3 className={css.language}>
              <p>
                <span className={css.span_transcript}>Chat</span>
              </p>
            </h3>
            {!chat && chat.length <= 0 && !ismic ? (
              <p>
                Click the mic to transcribe live in English or select another
                language.
              </p>
            ) : (
              <p>
                {chat.map((item, index) => (
                  <div key={index} className={css.chat_div}>
                    <p key={index + 1}>
                      <b>{item.speaker}</b> :{" "}
                      <Markdown>{item.message}</Markdown>
                    </p>
                    <div ref={msgref}></div>
                  </div>
                ))}
                <div ref={msgref2}></div>
              </p>
            )}
          </div>
        </div>
      </div>

      {isprompt && (
        <div className={`${css.prompt_popup}`}>
            <textarea
              name="prompt"
              id="prompt"
              cols="50"
              rows="30"
              value={chat_prompt}
              placeholder="Add Call Instructions..."
              onChange={(e)=>{
                setchat_prompt(e.target.value);
                sessionStorage.setItem("prompt" , e.target.value);
              }}
            ></textarea>
            <button
              className={`${css.prompt}`}
              onClick={() => {
                setisprompt(false);
              }}
            >
              Done
            </button>
        </div>
      )}
    </>
  );
}

export default Home;
